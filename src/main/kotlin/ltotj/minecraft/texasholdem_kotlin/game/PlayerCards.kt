...
class PlayerCards {
    ...
    private fun sortCardNum(sortedCards:ArrayList<Card>){
        for(i in 0..sortedCards.size)for(j in i+1..sortedCards.size){
    ...
    fun getHand(community:ArrayList<Card>):Double {//フラッシュ系統のみ1桁目はsuit その他は0
        ...
        if (dupSuit > 9) {//フラッシュ系統
            ...
            for (i in 0..7) if (judgedCards[i].suit == judgedCards[3].suit && i != 3) insCards.add(judgedCards[i])
            ...
            for (i in insCards.size - 5 downTo 0) {
                if (insCards[i + 4].num == 4) {
                    if (insCards[i + 4].num == 14) hand = 1914131211100.0 + insCards[0].suit
                    else hand = 1800000000000.0 + 1000000000 * insCards[i + 4].num + 10000000 * insCards[i + 3].num + 100000 * insCards[i + 2].num + 1000 * insCards[i + 1].num + 10 * insCards[i].num + insCards[0].suit
                    straight = true
                    break
                }
            }
            if (!straight) {
                hand = 1500000000000.0;
                for (i in insCards.size - 1 downTo insCards.size - 5) {
                    hand += insCards[i].num * 100.0.pow(4 + i - insCards.size) * 10
                }
            }
        }
        ...
        when (dupNum) {
            9, 7 -> hand = 1700000000000.0 + 1010101000 * judgedCards[3].num + 10 * (judgedCards[2].num + judgedCards[6].num - judgedCards[3].num)
            6 -> hand = if (judgedCards[1].num == judgedCards[2].num && judgedCards[4].num == judgedCards[5].num) 1600000000000.0 + 1010100000 * judgedCards[4].num + judgedCards[2].num * 1010
            else 1700000000000.0 + 1010101000 * judgedCards[3].num + 10 * (judgedCards[2].num + judgedCards[6].num - judgedCards[3].num)
            5, 4 -> {//FH
                var k=0
                for (i in 6 downTo 2) {
                    if (judgedCards[i].num == judgedCards[i - 2].num) {
                        hand = 1600000000000.0 + judgedCards[i].num * 1010100000
                        k = i
                        break
                    }
                }
        ...
        if (!straight && dupSuit < 10) {
            when (dupNum) {
                3 -> if ((judgedCards[6].num == judgedCards[5].num && judgedCards[6].num != judgedCards[4].num) || (judgedCards[0].num == judgedCards[1].num && judgedCards[0].num != judgedCards[2].num)) {
                    hand = 1200000000000.0 + judgedCards[5].num * 1010000000 + judgedCards[3].num * 101000 + 10 * (judgedCards[6].num + judgedCards[4].num + judgedCards[2].num - judgedCards[5].num - judgedCards[3].num)
                } else {
                    var k=0
                    for (i in 6 downTo 2) {
                        if (judgedCards[i].num == judgedCards[i - 1].num) {
                            hand = 1300000000000.0 + judgedCards[i].num * 1010100000;
                            k = i
                            break;
                        }
                    }
                    ...
                    for (i in 6 downTo 0) {
                        ...
                        hand += 100.0.pow(1 - count) * judgedCards[i].num * 10
                        count += 1
                        if (count == 2) break
                    }
                }
                2 -> {
                    ...
                    for (i in 6 downTo 3) {
                        if (judgedCards[i].num == judgedCards[i - 1].num) {
                            hand = 1200000000000.0 + 1010000000 * judgedCards[i].num
                            k = i
                            break
                        }
                    }
                ...
                1 -> {
                    ...
                    for (i in 6 downTo 1) {
                        if (judgedCards[i].num == judgedCards[i - 1].num) {
                            hand = 1100000000000.0 + judgedCards[i].num * 1010000000
                            k = i
                            break
                        }
                    }
                    ...
                    for (i in 6 downTo 0) {
                        if (judgedCards[i].num == judgedCards[k].num) continue
                        hand += 100.0.pow(2 - count) * judgedCards[i].num * 10
                        count += 1
                        if (count == 2) break
                    }
                }
                0 -> {
                    ...
                    for (i in 6 downTo 2) hand += 100.0.pow(i - 2) * judgedCards[i].num * 10
                }
            }
        }
