...
class PlayerGUI(private val seat: Int){
    ...
    fun setFaceDownCard(slot: Int){
        val material = Material.valueOf(plugin.config.getString("cardMaterial")!!)
        val item = ItemStack(material, 1)
        val meta = item.itemMeta
        meta.setCustomModelData(plugin.config.getInt("faceDownCard.customModelData"))
    ...
    fun setRaiseButton(){
        for(i in 45..52){
            setGUIItem(i, Material.valueOf(plugin.config.getString("raise.$i.material")!!), plugin.config.getString("raise.$i.name")!!)
        }
    }
    ...
    fun reloadRaiseButton(add:Int){
        setGUIItem(49, Material.valueOf(plugin.config.getString("raise.49.material")!!), plugin.config.getString("raise.49.name")!!, listOf("§c" + add + "枚追加"))
    }

    fun setActionButton(){
        for (i in 45..52) {
            setGUIItem(i, Material.valueOf(plugin.config.getString("texasholdeminv.$i.material")!!), plugin.config.getString("texasholdeminv.$i.name")!!, listOf(plugin.config.getString("texasholdeminv.$i.lore")!!))
        }
    }
