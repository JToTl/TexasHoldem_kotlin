    ...
    override fun onCommand(sender: CommandSender, command: Command, label: String, args: Array<out String>): Boolean{
        ...
        when(args[0]){
            "start"->{
                if(!playable.get())sender.sendMessage("[TexasHoldem]はただいま停止中です")
                ...
                else if(args.size<3||args[1].toIntOrNull()==null||args[2].toIntOrNull()==null||args[1].toInt()< Main.plugin.config.getInt("minChipRate")||
                        args[1].toInt()> Main.plugin.config.getInt("maxChipRate")|| abs(args[2].toInt() - 3) >1)sender.sendMessage("/poker start <チップ一枚あたりの金額:"+ Main.plugin.config.getDouble("minChipRate")+"以上> <最低募集人数:2〜4人> (最大募集人数:2〜4人)")
                else if(vault.getBalance(uuid)<args[1].toInt()* Main.plugin.config.getDouble("firstNumberOfChips"))sender.sendMessage("所持金が足りません")
                else{
                    ...
                    texasHoldemTables[uuid]?.addPlayer(sender)
                    Bukkit.broadcast(Component.text("§l"+sender.name+"§aが§cチップ一枚"+args[1]+"円§r、§l§e募集人数"+args[2]+"〜"+ texasHoldemTables[uuid]!!.maxSeat+"人、§c周回数"+ texasHoldemTables[uuid]!!.roundTimes+"回§aで§7§lテキサスホールデム§aを募集中！§r/poker join "+sender.name+" §l§aで参加しましょう！ §4注意 参加必要金額"+(args[1].toDouble()* Main.plugin.config.getDouble("firstNumberOfChips"))), Server.BROADCAST_CHANNEL_USERS)
            ...
            "help"->{
                sender.sendMessage("/poker start <チップ一枚あたりの金額:10000円以上> <最低募集人数:2〜4人> (最大募集人数:2〜4人) (周回数：１〜４):テキサスホールデムの参加者を募集します 参加人数分だけゲームが行われます §4注意 設定金額×"+ Main.plugin.config.getInt("firstNumberOfChips")+"が必要です")
